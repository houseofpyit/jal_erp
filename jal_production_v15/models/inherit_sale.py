...
class inheritedSaleOrderLine(models.Model):
    ...
    @api.onchange('bucket_cap_id','product_uom_qty','product_uom')
    def _onchange_bucket_cap_id(self):
        for rec in self:
            if rec.bucket_cap_id.weight > 0:
                rec.bucket = (rec.product_uom_qty * rec.product_uom.ratio) / rec.bucket_cap_id.weight
            else:
                rec.bucket = 0
